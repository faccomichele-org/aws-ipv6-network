locals {
  ...
  flow_log_tag_fields = concat(
    [for i in range(length(lookup(var.flow_logs_tag_keys, "instance", []))) : i == 0 ? "$${instance-tag}" : "$${instance-tag-2}"],
    [for i in range(length(lookup(var.flow_logs_tag_keys, "network-interface", []))) : i == 0 ? "$${interface-tag}" : "$${interface-tag-2}"],
    [for i in range(length(lookup(var.flow_logs_tag_keys, "auto-scaling-group", []))) : i == 0 ? "$${asg-tag}" : "$${asg-tag-2}"]
  )

  flow_log_format = join(" ", concat([
    "$${version}", "$${account-id}", "$${interface-id}", "$${srcaddr}", "$${dstaddr}", "$${srcport}", "$${dstport}", "$${protocol}", "$${packets}", "$${bytes}", "$${start}", "$${end}", "$${action}", "$${log-status}", "$${vpc-id}", "$${subnet-id}", "$${instance-id}", "$${tcp-flags}", "$${type}", "$${pkt-srcaddr}", "$${pkt-dstaddr}", "$${region}", "$${az-id}", "$${sublocation-type}", "$${sublocation-id}", "$${pkt-src-aws-service}", "$${pkt-dst-aws-service}", "$${flow-direction}", "$${traffic-path}", "$${ecs-cluster-arn}", "$${ecs-cluster-name}", "$${ecs-service-name}", "$${ecs-task-arn}", "$${ecs-task-id}", "$${ecs-task-definition-arn}", "$${ecs-container-instance-arn}", "$${ecs-container-instance-id}", "$${ecs-container-id}", "$${ecs-second-container-id}", "$${reject-reason}", "$${resource-id}", "$${encryption-status}"
    ], local.flow_log_tag_fields, [
    "$${interface-type}", "$${next-hop-interface-id}", "$${next-hop-subnet-id}", "$${next-hop-az-id}", "$${next-hop-vpc-id}", "$${next-hop-interface-type}"
  ]))
