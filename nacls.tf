...
resource "aws_network_acl" "services" {
  ...
  tags = merge(local.tags,
    {
      Name = "${local.project_name}-services-nacl"
      File = "nacls.tf"
    }
  )
}
...
resource "aws_network_acl_rule" "services_ingress_ipv6_ephemeral" {
  network_acl_id  = aws_network_acl.services.id
  rule_number     = 120
  egress          = false
  protocol        = "6"
...
resource "aws_network_acl_rule" "services_egress_ipv6_https" {
  network_acl_id  = aws_network_acl.services.id
  rule_number     = 120
  egress          = true
  protocol        = "6"
...
resource "aws_network_acl_rule" "services_egress_ipv6_ssh" {
  network_acl_id  = aws_network_acl.services.id
  rule_number     = 130
  egress          = true
  protocol        = "6"
